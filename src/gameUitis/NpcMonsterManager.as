	...
	public class NpcMonsterManager
	{
		public function getMonster(hpMax:Number = 20,attake:Number = 2,defense:Number = 3,controMode:int = 2):int
		{
			for(var i:int = 1; i<99 ; i++)
			{
				if(!_Monster[i])
				{
					_Monster[i] =new NpcMonster(hpMax,attake,defense,false,controMode);   
		...
		public function getNpc(hpMax:Number = 10,attake:Number = 2,defense:Number = 2,controMode:int = 0):int
		{
			for(var i:int = 1; i<99 ; i++)
			{
				if(!_Npc[i])
				{
					_Npc[i] =new NpcMonster(hpMax,attake,defense,true,controMode);   
